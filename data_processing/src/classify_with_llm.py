#!/usr/bin/env python3
"""
Asynchronná klasifikácia webových stránok s opravou API Key, Batchingu a
incrementálnym zápisom výsledkov po každom batchi.
"""
        ...
        try:
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    print(
                        f"Rate limit hit (attempt {attempt + 1}), čakám...",
                        file=sys.stderr,
                    )
                    ...
                    print(
                        "API Error "
                        f"{response.status} pre text (krátky): {text[:50]}... "
                        f"Chyba: {error_text}",
                        file=sys.stderr,
                    )
                    ...
                    if (
                            response.status == 400
                            and "context length" in error_text.lower()
                    ):
                        if current_max_input_chars > 4000 and attempt < retries - 1:
                            current_max_input_chars = max(
                                4000, current_max_input_chars // 2
                            )
                            print(
                                "Text je príliš dlhý, skúšam kratší vstup "
                                f"({current_max_input_chars} znakov).",
                                file=sys.stderr,
                            )
                            continue
...
async def main_async(args):
    ...
    if not base_url:
        print(
            "Chyba: base_url musí byť nastavené cez --base-url "
            "alebo OPENAI_BASE_URL.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(
        "Pripravených "
        f"{total_lines} záznamov. API URL: {base_url}, "
        f"API Key nastavená: "
        f"{'Áno' if api_key and api_key != 'not-needed' else 'Nie'}"
    )

    # Otvoríme výstupný súbor hneď na začiatku pre inkrementálny zápis
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            for batch_idx, batch in enumerate(batches):
                    ...
                    if not text:
                        if compressed_mode:
                            out = {
                                "url": url,
                                "categories": [],
                                "compressed_html": item.get("compressed_html", ""),
                            }
                        else:
                            out = {
                                "url": url,
                ...
                gathered = await asyncio.gather(
                    *(t[1] for t in tasks_with_indices), return_exceptions=True
                )
                ...
                for local_idx, res in local_results.items():
                    ...
                    try:
                        ...
                        print(f"Chyba pri zápise riadku {global_idx}: {e}", file=sys.stderr)

                        # Synchronizácia na disk (flush + fsync)
                fout.flush()
        ...
        # Uzatvorenie súboru v finally bloku pre istotu
        fout.close()

    print(f"\nHotovo. Celkový počet riadkov vo výstupe: {len(items)}")
...
def main():
    ...
    parser.add_argument("--batch-size", type=int, default=10,
                        help="Veľkosť batchu (počet paralelných požiadaviek)", )
    parser.add_argument("--compressed-jsonl", action="store_true",
                        help=(
                            "Vstup aj výstup budú mať formát: "
